...
const getSupervisor = async (req, res) => {
    ...
    const supervisor = await Supervisor.findOne({ user: userId })
        ...
        .populate('assignedStudents', 'name')
    if (!supervisor) {
        return res.status(404).json({ error: 'No supervisor found with this user ID' })
    }

    // Check if the requesting user is the supervisor themselves or an admin
    if (supervisor.user._id.toString() !== req.user.id && req.user.role !== 'admin') {
        return res.status(403).json({ error: 'Not authorized to access this supervisor' })
    }
...
const updateSupervisor = async (req, res) => {
    ...
    try {
        const supervisor = await Supervisor.findOneAndUpdate({ user: userId }, updateData, { new: true }).populate('user faculty')
        if (!supervisor) {
